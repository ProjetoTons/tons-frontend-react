    ...
    <div className="w-full bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col">
        {funcionarios.map((func, index) => (
          <div 
            key={func.id} 
            className={`flex items-center px-6 py-4 text-sm text-gray-700 ${
              index !== funcionarios.length - 1 ? "border-b border-gray-100" : ""
          ...
          >
            ...
            {/* Cargo (Badge) */}
            <div className="w-[15%]">
              <span className="bg-[#EFEFEF] text-gray-600 px-3 py-1 text-[9px] font-bold uppercase tracking-widest rounded-sm">
                {func.cargo}
              </span>
            </div>

            {/* Data Nascimento */}
            <div className="w-[15%] text-gray-500">{func.dataNascimento}</div>

            {/* Ações (Editar / Excluir) */}
            <div className="w-[5%] flex justify-center gap-3 text-gray-400">
              <button onClick={() => onEdit(func.id)} className="hover:text-black transition-colors" title="Editar">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                </svg>
              </button>
              <button onClick={() => onDelete(func.id)} className="hover:text-red-500 transition-colors" title="Excluir">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
